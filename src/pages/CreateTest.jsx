...
export default function CreateTest() {
  ...
  const inputStyle = { width: '100%', padding: '12px 16px', background: 'rgba(30,41,59,0.8)', border: '1px solid rgba(51,65,85,0.5)', borderRadius: 12, color: '#f1f5f9', fontSize: 14, outline: 'none', boxSizing: 'border-box' };
  const letters = ['A', 'B', 'C'];
    ...
    <div style={{ minHeight: '100vh', background: '#020617' }}>
      ...
      <div style={{ padding: '32px 16px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ marginBottom: 32 }}>
          <p style={{ color: '#10b981', fontSize: 13, fontWeight: 500, textTransform: 'uppercase', letterSpacing: 2, marginBottom: 8 }}>📝 Test Tuzish</p>
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 36, fontWeight: 700, color: '#f1f5f9' }}>Yangi test yarating</h1>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 32, alignItems: 'start' }}>
          ...
          <div style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(51,65,85,0.5)', borderRadius: 20, padding: 24, position: 'sticky', top: 80 }}>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 18, fontWeight: 700, color: '#f1f5f9', marginBottom: 24 }}>❓ Savol qo'shish</h2>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div>
                <label style={{ display: 'block', color: '#cbd5e1', fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Test savoli</label>
              ...
              <div>
                <label style={{ display: 'block', color: '#cbd5e1', fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Javob variantlari</label>
                  ...
                  {options.map((opt, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <button type="button" onClick={() => setCorrectIndex(i)} style={{
                        ...
                        color: correctIndex === i ? '#10b981' : '#64748b', cursor: 'pointer', fontSize: 13, fontWeight: 700
                      }}>
                ...
                <p style={{ color: '#475569', fontSize: 12, marginTop: 8 }}>✓ To'g'ri javobni belgilash uchun chapdagi tugmani bosing</p>
              </div>
              <div>
                <label style={{ display: 'block', color: '#cbd5e1', fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Qiyinlik darajasi</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  {['easy', 'medium', 'hard'].map(d => (
                    <button key={d} type="button" onClick={() => setDifficulty(d)} style={{
                      ...
                      color: difficulty === d ? diffColors[d] : '#64748b', fontSize: 14, fontWeight: 500, cursor: 'pointer'
                    }}>
                      {diffLabels[d]}
                    </button>
                  ))}
              ...
              <button type="submit" disabled={submitting} style={{ width: '100%', padding: '12px 24px', background: '#10b981', color: '#020617', fontWeight: 600, fontSize: 14, borderRadius: 12, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.5 : 1 }}>
          ...
          {/* Testlar ro'yxati */}
          <div>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 18, fontWeight: 700, color: '#f1f5f9', marginBottom: 24 }}>📚 Barcha testlar ({tests.length})</h2>
            {loading ? <p style={{ color: '#64748b', textAlign: 'center' }}>Yuklanmoqda...</p> : tests.length === 0 ? (
              <div style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(51,65,85,0.5)', borderRadius: 20, padding: 48, textAlign: 'center' }}>
                ...
                <h3 style={{ color: '#cbd5e1', fontSize: 18, fontWeight: 500, marginBottom: 8 }}>Hali testlar yo'q</h3>
                <p style={{ color: '#475569' }}>Yuqoridagi forma orqali birinchi testni qo'shing</p>
              </div>
            ) : (
                    ...
                    {editingId === test.id ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        {editForm.options.map((opt, oi) => (
                          <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <button onClick={() => setEditForm({ ...editForm, correctIndex: oi })} style={{
                              ...
                              color: editForm.correctIndex === oi ? '#10b981' : '#64748b', cursor: 'pointer', fontSize: 13, fontWeight: 700
                            }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          {['easy', 'medium', 'hard'].map(d => (
                            <button key={d} onClick={() => setEditForm({ ...editForm, difficulty: d })} style={{
                              ...
                              color: editForm.difficulty === d ? diffColors[d] : '#64748b', fontSize: 14, fontWeight: 500, cursor: 'pointer'
                            }}>
                              {diffLabels[d]}
                            </button>
                          ))}
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                          <button onClick={() => handleSaveEdit(test.id)} style={{ padding: '8px 16px', background: '#10b981', color: '#020617', fontWeight: 600, borderRadius: 12, border: 'none', cursor: 'pointer', fontSize: 14 }}>💾 Saqlash</button>
                          <button onClick={() => { setEditingId(null); setEditForm(null); }} style={{ padding: '8px 16px', background: 'rgba(51,65,85,0.8)', color: '#f1f5f9', fontWeight: 500, borderRadius: 12, border: '1px solid rgba(51,65,85,0.5)', cursor: 'pointer', fontSize: 14 }}>✕ Bekor qilish</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                              <span style={{ padding: '4px 10px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: `${diffColors[test.difficulty]}15`, color: diffColors[test.difficulty], border: `1px solid ${diffColors[test.difficulty]}30` }}>
                                {diffLabels[test.difficulty]}
                              </span>
                              <span style={{ color: '#475569', fontSize: 12 }}>{test.creatorName} tomonidan</span>
                            </div>
                            <h3 style={{ color: '#f1f5f9', fontWeight: 500, marginBottom: 12, fontSize: 15 }}>{test.question}</h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              {test.options.map((opt, oi) => (
                                <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 8, fontSize: 14, background: oi === test.correctIndex ? 'rgba(16,185,129,0.1)' : 'transparent', color: oi === test.correctIndex ? '#10b981' : '#64748b', border: oi === test.correctIndex ? '1px solid rgba(16,185,129,0.2)' : 'none' }}>
                        ...
                        {deleteConfirm === test.id && (
                          <div style={{ marginTop: 16, padding: 16, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12 }}>
                            <p style={{ color: '#ef4444', fontSize: 14, fontWeight: 500, marginBottom: 12 }}>⚠️ Haqiqatanham o'chirmoqchimisiz?</p>
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button onClick={() => handleDelete(test.id)} style={{ padding: '8px 16px', background: 'rgba(239,68,68,0.15)', color: '#ef4444', fontWeight: 500, borderRadius: 12, border: '1px solid rgba(239,68,68,0.2)', cursor: 'pointer', fontSize: 14 }}>Ha, o'chirish</button>
                              <button onClick={() => setDeleteConfirm(null)} style={{ padding: '8px 16px', background: 'rgba(51,65,85,0.8)', color: '#f1f5f9', fontWeight: 500, borderRadius: 12, border: '1px solid rgba(51,65,85,0.5)', cursor: 'pointer', fontSize: 14 }}>Bekor qilish</button>
                            </div>
                          </div>
                        )}
                      </>
