    ...
    <div style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(51,65,85,0.5)', borderRadius: 20, padding: 24, fontFamily: fonts.body }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          {filters.map(f => (
            <button key={f.value} onClick={() => setFilter(f.value)} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: filter === f.value ? 'rgba(16,185,129,0.1)' : 'transparent', color: filter === f.value ? '#10b981' : '#475569', border: filter === f.value ? '1px solid rgba(16,185,129,0.2)' : '1px solid transparent', cursor: 'pointer', fontFamily: fonts.display }}>{f.label}</button>
          ))}
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>{results.map((r, i) => (
              <tr key={r.id} style={{ borderBottom: '1px solid rgba(30,41,59,0.5)' }}>
                ...
                <td style={{ padding: '12px 8px', color: '#cbd5e1', fontWeight: 600, fontSize: 14, fontFamily: fonts.display }}>{r.userName || "Noma'lum"}</td>
